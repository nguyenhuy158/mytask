...
async def test_send_notification_telegram(http_adapter):
    ...
    with patch("requests.post") as mock_post:
        await http_adapter.send_notification(
            [target], "Title", "Message", level="success"
        )
        mock_post.assert_called_once()
        args, kwargs = mock_post.call_args
...
async def test_send_notification_slack(http_adapter):
    ...
    with patch("requests.post") as mock_post:
        await http_adapter.send_notification(
            [target], "Title", "Message", level="error"
        )
        mock_post.assert_called_once()
        args, kwargs = mock_post.call_args
...
async def test_send_notification_warning(http_adapter):
    ...
    with patch("requests.post") as mock_post:
        await http_adapter.send_notification(
            [target], "Title", "Message", level="warning"
        )
        mock_post.assert_called_once()
...
def test_send_telegram_with_url(http_adapter):
    with patch("requests.post") as mock_post:
        http_adapter._send_telegram(
            "http://telegram.url", None, None, "Title", "Message", "ℹ️"
        )
        mock_post.assert_called_once_with(
            "http://telegram.url", json={"text": "ℹ️ *Title*\n\nMessage"}, timeout=5
        )
