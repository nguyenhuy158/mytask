...
from app.core.entities.models import TaskSchema, S3ConfigSchema, NotificationConfigSchema
...
async def test_s3_configs(prisma_adapter):
    config = S3ConfigSchema(
        name="S3", endpoint="http://e", region="r", bucket="b", access_key="a", secret_key="s"
    )
...
async def test_attachments(prisma_adapter):
    ...
    att = FileAttachmentSchema(name="f.txt", key="k", bucket="b", mimetype="text/plain", task_id=task.id)
