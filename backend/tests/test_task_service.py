...
async def test_create_task(task_service, mock_repo, mock_broadcast):
    ...
    mock_repo.create_task.return_value = MagicMock(
        id=1, name="Test", model_dump=lambda: {"id": 1, "name": "Test"}
    )
...
async def test_update_task_status_basic(task_service, mock_repo, mock_broadcast):
    mock_repo.update_task_status.return_value = MagicMock(
        id=1, name="Test", odoo_env_id=None
    )
...
async def test_execute_task_weather(task_service, mock_repo, mock_external_api):
    ...
    mock_repo.create_task_history.return_value = MagicMock(
        id=1, task_id=1, timestamp=datetime.now()
    )
...
@pytest.mark.asyncio
async def test_get_history(task_service, mock_repo):
    mock_repo.get_history.return_value = []
    result = await task_service.get_history()
    assert result == []


@pytest.mark.asyncio
async def test_create_note(task_service, mock_repo):
    mock_repo.create_note.return_value = MagicMock()
    await task_service.create_note({"title": "N"})
    mock_repo.create_note.assert_called_once()


@pytest.mark.asyncio
async def test_list_notes(task_service, mock_repo):
    mock_repo.get_notes.return_value = []
    result = await task_service.list_notes()
    assert result == []


@pytest.mark.asyncio
async def test_get_attachment_url_error(task_service, mock_repo):
    mock_repo.get_attachment_by_id.return_value = None
    with pytest.raises(ValueError):
        await task_service.get_attachment_url(999)
